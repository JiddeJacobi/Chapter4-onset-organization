...
## Read data
```{r read}
df <- read.csv("C:/Users/Jidde/Desktop/analysis/data/c-center_16th_Sep2.csv", header = TRUE, sep = ",")
load("C:/Users/Jidde/Desktop/analysis/data/speech_rate.Rda")
df_sr <- drop_na(df_sr)
```
...
Change level of group factor:
```{r}
df$group <- recode_factor(df$group, CTRL = "TP")
```
...
sp.CC <- df %>%
  ...
  mutate(time.lag.norm = time.lag/duration.tt)  %>%
  mutate(dur.C1 = (t5.s - t3.s)) %>%
  mutate(dur.C2 = (t5.p - t3.p))  %>%
  mutate(dur.C1.norm = dur.C1/duration.tt) %>%
  mutate(dur.C2.norm = dur.C2/duration.tt)



sp.C1.plot <- ggplot(sp.CC, aes(x = time.lag.norm, y = dur.C1.norm, color = group, shape = group)) + geom_point(size=2)
print(sp.C1.plot + scale_shape_manual(values = c(0, 2)) + scale_colour_manual(values = c("seagreen4", "coral2")) + theme_bw() + labs(x = "normalized IPI", y = "normalized duration C1")) + geom_smooth(method='lm', se = FALSE) +   xlim(-0.15, 0.3) + ylim(0, 0.3)

cor.test(sp.CC[sp.CC$group == "TP",]$time.lag.norm, sp.CC[sp.CC$group == "TP",]$dur.C1.norm)
cor.test(sp.CC[sp.CC$group == "PD",]$time.lag.norm, sp.CC[sp.CC$group == "PD",]$dur.C1.norm)



sp.C2.plot <- ggplot(sp.CC, aes(x = time.lag.norm, y = dur.C2.norm, color = group, shape = group)) + geom_point(size=2)
print(sp.C2.plot + scale_shape_manual(values = c(0, 2)) + scale_colour_manual(values = c("seagreen4", "coral2")) + theme_bw() + labs(x = "normalized IPI", y = "normalized duration C2")) + geom_smooth(method='lm', se = FALSE) +   xlim(-0.15, 0.3) + ylim(0, 0.3)

cor.test(sp.CC[sp.CC$group == "TP",]$time.lag.norm, sp.CC[sp.CC$group == "TP",]$dur.C2.norm)
cor.test(sp.CC[sp.CC$group == "PD",]$time.lag.norm, sp.CC[sp.CC$group == "PD",]$dur.C2.norm)
...
print(sm.C1.plot + scale_shape_manual(values = c(0, 2)) + scale_colour_manual(values = c("seagreen4", "coral2")) + theme_bw() + labs(x = "normalized IPI", y = "normalized duration C1")) + geom_smooth(method='lm', se = FALSE) +   xlim(-0.15, 0.3) + ylim(0, 0.3)

cor.test(sm.CC[sm.CC$group == "TP",]$time.lag.norm, sm.CC[sm.CC$group == "TP",]$dur.C1.norm)
cor.test(sm.CC[sm.CC$group == "PD",]$time.lag.norm, sm.CC[sm.CC$group == "PD",]$dur.C1.norm)


sm.C2.plot <- ggplot(sm.CC, aes(x = time.lag.norm, y = dur.C2.norm, color = group, shape = group)) + geom_point(size=2)
print(sm.C2.plot + scale_shape_manual(values = c(0, 2)) + scale_colour_manual(values = c("seagreen4", "coral2")) + theme_bw() + labs(x = "normalized IPI", y = "normalized duration C2")) + geom_smooth(method='lm', se = FALSE) +   xlim(-0.15, 0.3) + ylim(0, 0.3)

cor.test(sm.CC[sm.CC$group == "TP",]$time.lag.norm, sm.CC[sm.CC$group == "TP",]$dur.C2.norm)
cor.test(sm.CC[sm.CC$group == "PD",]$time.lag.norm, sm.CC[sm.CC$group == "PD",]$dur.C2.norm)
...
print(sx.C1.plot + scale_shape_manual(values = c(0, 2)) + scale_colour_manual(values = c("seagreen4", "coral2")) + theme_bw() + labs(x = "normalized IPI", y = "normalized duration C1")) + geom_smooth(method='lm', se = FALSE) +   xlim(-0.15, 0.3) + ylim(0, 0.3)

cor.test(sx.CC[sx.CC$group == "TP",]$time.lag.norm, sx.CC[sx.CC$group == "TP",]$dur.C1.norm)
cor.test(sx.CC[sx.CC$group == "PD",]$time.lag.norm, sx.CC[sx.CC$group == "PD",]$dur.C1.norm)

sx.C2.plot <- ggplot(sx.CC, aes(x = time.lag.norm, y = dur.C2.norm, color = group, shape = group)) + geom_point(size=2)
print(sx.C2.plot + scale_shape_manual(values = c(0, 2)) + scale_colour_manual(values = c("seagreen4", "coral2")) + theme_bw() + labs(x = "normalized IPI", y = "normalized duration C2")) + geom_smooth(method='lm', se = FALSE) +   xlim(-0.15, 0.3) + ylim(0, 0.3)

cor.test(sx.CC[sx.CC$group == "TP",]$time.lag.norm, sx.CC[sx.CC$group == "TP",]$dur.C2.norm)
cor.test(sx.CC[sx.CC$group == "PD",]$time.lag.norm, sx.CC[sx.CC$group == "PD",]$dur.C2.norm)
...
spr.CC <- df %>%
  ...
  mutate(time.lag.norm = time.lag/duration.tt) %>%
  mutate(dur.C1.norm = (t5.s - t3.s)/duration.tt)  %>%
  mutate(dur.C2.norm = (t5.p - t3.p)/duration.tt)  %>%
  mutate(dur.C3.norm = (t5.r - t3.r)/duration.tt)
...
# Check how many trials are used in C-center data
```{r}

# create df without spr
df.C.center.nospr <- df.C.center [! df.C.center$cluster == "spr", ]
df.C.center.nospr <- droplevels(df.C.center.nospr)
levels(df.C.center.nospr$cluster)

df.C.center.nospr <- df.C.center.nospr %>% drop_na(leftedge.lag, rightedge.lag, c.center.lag)

df.C.center.speech.rate <- df.C.center.nospr %>%
  group_by(subj, cluster) %>%
  summarize(mean.sr = mean(duration.tt))
```
...
df.RSD.long <-  merge(df.RSD.long, df.C.center.speech.rate, by=c("subj", "cluster"))
